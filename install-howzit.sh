...
echo -e "  - Captive portal will receive traffic from: ${ETHERNET_INTERFACE}"
...
# Create network configuration script - UPDATED VERSION
...
restart_networking() {
  ...
  if systemctl list-unit-files | grep -q "networking.service"; then
    systemctl restart networking || log_message "Failed to restart networking, but continuing..."
  elif systemctl list-unit-files | grep -q "dhcpcd.service"; then
    systemctl restart dhcpcd || log_message "Failed to restart dhcpcd, but continuing..."
  elif systemctl list-unit-files | grep -q "NetworkManager.service"; then
    systemctl restart NetworkManager || log_message "Failed to restart NetworkManager, but continuing..."
  else
    log_message "Warning: No recognized networking service found. Restarting interfaces directly."
...
log_message "Stopping network services before reconfiguration..."
systemctl stop hostapd dnsmasq || true

# Configure hostapd for WiFi access point
log_message "Configuring hostapd for WiFi access point..."
...
if [ -d "/etc/network/interfaces.d" ]; then
    ...
    log_message "Using Debian-style network configuration..."
    
    mkdir -p /etc/network/interfaces.d
    
    # Configure WiFi interface (AP mode)
...
EOF
    ...
    # Configure Ethernet interface (DHCP client)
...
EOF
    ...
    log_message "Using dhcpcd for network configuration..."
    
    # Back up the original config
    cp /etc/dhcpcd.conf /etc/dhcpcd.conf.backup
    
    # Configure static IP for WiFi interface and DHCP for ethernet
    cat > /etc/dhcpcd.conf << EOF
# Configuration added by Howzit installer
# Allow DHCP to assign address to ethernet interface
interface ${ETHERNET_INTERFACE}

# Static IP configuration for WiFi interface
interface ${WIFI_INTERFACE}
...
EOF
    ...
    log_message "Warning: Could not detect network configuration method. Manual configuration may be required."
fi

# Reset all iptables rules to start fresh
log_message "Resetting iptables chains and rules..."
iptables -F
iptables -X
iptables -t nat -F
iptables -t nat -X
...
log_message "Configuring IP forwarding and NAT..."
echo 1 > /proc/sys/net/ipv4/ip_forward
echo "net.ipv4.ip_forward=1" > /etc/sysctl.d/90-ip-forward.conf
sysctl -p /etc/sysctl.d/90-ip-forward.conf

# Configure iptables for NAT and captive portal
log_message "Configuring iptables for captive portal..."
...
log_message "Enabling and starting network services..."
...
if systemctl is-active --quiet hostapd; then
    log_message "hostapd service started successfully"
else
    log_message "Warning: hostapd service failed to start"
fi

if systemctl is-active --quiet dnsmasq; then
    log_message "dnsmasq service started successfully"
else
    log_message "Warning: dnsmasq service failed to start"
fi

log_message "Network configuration completed."
EOL
...
echo -e "Internet traffic from the WiFi clients will be forwarded through the ethernet port (${ETHERNET_INTERFACE})."
