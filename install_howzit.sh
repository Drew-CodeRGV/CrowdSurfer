...
echo -e "\n\033[32mHowzit Captive Portal Installation Script - v1.2.5\033[0m\n"
...
echo "[Howzit] Waiting for $CP_INTERFACE to come up and get IP 10.69.0.1..."
while true; do
  if ip addr show $CP_INTERFACE | grep -q "inet 10.69.0.1"; then
    echo "[Howzit] $CP_INTERFACE is up and has 10.69.0.1. Proceeding."
    break
  fi
  echo "[Howzit] $CP_INTERFACE not ready. Retrying in 5 seconds..."
  ip link set $CP_INTERFACE up
  ip addr flush dev $CP_INTERFACE
  ip addr add 10.69.0.1/24 dev $CP_INTERFACE
  sleep 5
